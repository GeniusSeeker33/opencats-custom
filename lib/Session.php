...
class CATSSession
{
    ...
    public function processLogin($username, $password, $addToHistory = true)
    {
		...
		// Setup session cookie values safely
		...
		// For PHP < 7.3, use header() to set cookie manually
		header("Set-Cookie: session_cookie=$cookieValue; expires=" . gmdate('D, d-M-Y H:i:s T', $expires) . "; path=$path; Secure; 		HttpOnly; SameSite=$samesite");

		// Update the user session in the database
		$sql = sprintf(
    		"UPDATE user SET force_logout = 0 WHERE user_id = %s AND site_id = %s",
