...
class ImportUI extends UserInterface
{
    ...
    */
	...
	if (strtolower($extension) === 'pdf') {
    	    try {
            	$parser = new Parser();
            	$pdf = $parser->parseFile($tempFilename);
            	$parsedText = $pdf->getText();

        	// Save the parsed resume text to a temporary text file
        	$textOutputPath = $tempFilename . '.txt';
        	file_put_contents($textOutputPath, $parsedText);

        	// Optional: store parsed text in session or log
        	$_SESSION['parsedResumeText'] = $parsedText;
        	error_log("✅ Parsed resume text from: $originalFilename");

    } catch (Exception $e) {
        error_log("❌ Resume parsing failed for $originalFilename: " . $e->getMessage());
    }
}
